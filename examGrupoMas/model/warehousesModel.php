...
class WarehousesModel extends DatabaseConnector {
    ...
    public function getWarehousesInformation() {
        ...
        try {
            $sql = "SELECT warehouses.id AS warehouse_id, warehouses.name, count(item_warehouse.item_id) as noItems, IF(count(item_warehouse.item_id) = 0, 'Sin items', IF(in_stock.elements IS NULL, 'En stock','Falta stock')) as inStock";
            $sql .= " FROM warehouses";
